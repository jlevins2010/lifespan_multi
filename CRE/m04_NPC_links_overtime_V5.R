...
chunk_size         <- 250000  # Number of pairs to process per batch
nGenes             <- 1000     
...
dar_file    <- "/vast/home/l/levinsj/DARs/Fetal_NPC_vs_All_Adult_DARs.csv"
#dar_file <- "/vast/home/l/levinsj/DARs/fetalNPC_compare_other_adultCells_pseudoBulk_DARs.csv"

base_output_dir <- "/vast/home/l/levinsj/output_csv/"
dir.create(base_output_dir, recursive = TRUE, showWarnings = FALSE)

merged_output          <- paste0(base_output_dir, "mergedMultiMapoutput_fetalNPClinks_human.csv")
merged_output_filtered <- paste0(base_output_dir, "mergedMultiMapoutput_fetalNPClinks_filtered_human.csv")
...
# Convert filtered peak strings into GRanges
peaks_granges_full <- Signac::StringToGRanges(dar_filtered$peak, sep = c(":", "-"))

# Assign custom IDs in standard hyphenated format ('chr-start-end')
...
# CRITICAL FIX: Assign names to GRanges so DistanceToTSS populates rownames correctly
names(peaks_granges_full) <- peaks_granges_full$custom_id
...
# Calculates TSS distances using DAR-filtered peaks GRanges object
...
message(paste0("Number of gene-peak pairs within window: ", nrow(df_pairs)))
...
for (comp in comparison_targets) {
  ...
  output_file <- paste0(base_output_dir, "fetalNPClinks_", comp_name, "_multiMapoutput_human.csv")
...
desired_order <- c("Adult_PT", "Pediatric_PT", "Fetal_PT", "Fetal_Int", "Fetal_NPC")
cellTypes     <- c("Adult_PT", "Pediatric_PT", "Fetal_PT", "Fetal_Int", "Fetal_NPC")
filenames     <- map_chr(cellTypes, ~ str_c(base_output_dir, "fetalNPClinks_", .x, "_multiMapoutput_human.csv"))
...
plot_count_data <- function(df, metric, cutoff, compare_fn, title_prefix, file_suffix, y_label) {
  ...
  ggsave(
    filename = str_c("/vast/home/l/levinsj/plots/multiOme/fetal_NPC_human_", file_suffix, ".eps"),
    device = cairo_ps,
    plot = sig_plot,
    width = 8,
    height = 10,
...
message("--- Generating Fetal_NPC Correlation Conservation Scatter Plots ---")

# Filter for significant Fetal NPC links
sig_fetal_npc_df <- final_merged_df %>%
  filter(!is.na(padj_Fetal_NPC) & padj_Fetal_NPC < 0.05)

# CRITICAL FIX: Align reference column with Fetal_NPC and pivot comparison groups
scatter_df <- sig_fetal_npc_df %>%
  ...
  filter(Comparison_Group != "Fetal_NPC") # Exclude self-comparison

if (nrow(scatter_df) == 0 || all(is.na(scatter_df$cor_Fetal_NPC))) {
  stop("scatter_df is empty or contains only NA values. Check upstream processing step.")
}
...
scatter_df <- scatter_df %>%
  mutate(
    Comparison_Group = factor(
      Comparison_Group, 
      levels = c("Fetal_Int", "Fetal_PT", "Pediatric_PT", "Adult_PT")
    )
  )

# Calculate R^2 safely with fallback for NA handling
r2_labels <- scatter_df %>%
  ...
  summarize(
    r2_label = if (n() >= 2) {
      ...
      if (!is.na(r_val)) sprintf("R^2 == %.3f", r_val^2) else "R^2 == 'NA'"
    } else {
      "R^2 == 'NA'"
    },
    .groups = "drop"
...
# Plot Fetal_NPC correlations vs other groups
np_cor_scatter <- ggplot(scatter_df, aes(x = cor_Fetal_NPC, y = Correlation_Other)) +
  ...
  labs(
    ...
    subtitle = paste0("Filtered for significant Fetal NPC links (padj < 0.05, n = ", nrow(sig_fetal_npc_df), ")"),
...
ggsave(
  filename = "/vast/home/l/levinsj/plots/multiOme/adult_enhancers_human_cor_conservation_fetalNPC.eps",
  device = cairo_ps,
  plot = np_cor_scatter,
  width = 9,
  height = 8,
