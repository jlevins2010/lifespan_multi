...
dar_file    <- "/vast/home/l/levinsj/DARs/mouse_adultPT_allOtheradultcells_DAR.csv"

base_output_dir <- "/vast/home/l/levinsj/output_csv/"
dir.create(base_output_dir, recursive = TRUE, showWarnings = FALSE)

merged_output          <- paste0(base_output_dir, "mergedMultiMapoutput_adultPTlinks_mouse.csv")
merged_output_filtered <- paste0(base_output_dir, "mergedMultiMapoutput_adultPTlinks_filtered_mouse.csv")
...
# Identify "other" cells (none of the 6 populations)
...
for (comp in comparison_targets) {
  ...
  output_file <- paste0(base_output_dir, "adultPTlinks_", comp_name, "_multiMapoutput_mouse.csv")
...
filenames <- map_chr(cellTypes, ~ str_c(base_output_dir, "adultPTlinks_", .x, "_multiMapoutput_mouse.csv"))
...
plot_count_data <- function(df, metric, cutoff, compare_fn, title_prefix, file_suffix, y_label) {
  ...
  ggsave(
    filename = str_c("/vast/home/l/levinsj/plots/multiOme/adult_enhancers_mouse_", file_suffix, ".eps"),
    device = cairo_ps,
    plot = sig_plot,
    width = 8,
    height = 10,
...
# 4. Save plot
ggsave(
  filename = "/vast/home/l/levinsj/plots/multiOme/adult_enhancers_mouse_cor_conservation.eps",
  device = cairo_ps,
  plot = np_cor_scatter,
  width = 9,
  height = 8,
