...
workdir=/home/levinsj/MultiOme/Analysis/SALSA/WASP_ATAC/bcfilter_atac/${library_id} ### line changed @10jll_2
echo $workdir
echo $barcodes
rm -rf $workdir
rm -rf ${outputdir}/${outputbam}
mkdir -p $outputdir
mkdir -p $workdir
...
awk -v a="${library_id}" '{if($2 == a) {print $1"-1"}}' > /home/levinsj/MultiOme/Analysis/SALSA/work_dir/temp/temp_barcodes.${library_id}.txt

subset-bam \
  --bam $inputbam \
  --cell-barcodes /home/levinsj/MultiOme/Analysis/SALSA/work_dir/temp/temp_barcodes.${library_id}.txt \
